...
public class IngestPoller implements Runnable
{
	...
	try
	{
		...
		for(ListBlobItem blobItem : container.listBlobs("processing", true, null, null, null))
		{
		...
		// ---------------
		String copyDestination = blob.getName().replace("landing", "processing");
		...
		if(destination.exists() && destination.getCopyState().getStatus().equals(CopyStatus.SUCCESS))
		{
			...
			// put xml on the queue here
			try
			{
				
			IngesterTransferJson ingesterTransferJson = new IngesterTransferJson();
			ingesterTransferJson.setUri(destination.getUri());
			ingesterTransferJson.setAzureBlobInfo(source);
			ingesterTransferJson.setBlobName(destination.getName());
			Gson gson = new Gson();
			String ingesterJson = gson.toJson(ingesterTransferJson);
			
			queueManager.push(Util.getConfigProperty("redis.xml.parser.input.list"), ingesterJson);
			blob.deleteIfExists();
			}
