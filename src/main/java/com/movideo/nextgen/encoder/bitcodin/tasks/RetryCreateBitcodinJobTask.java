...
public class RetryCreateBitcodinJobTask extends Task {
	...
	try {
		...
		try {
		    ...
		    long waitTime = getWaitTimeExp(job.getRetryCount());
		    if (waitTime < MAX_WAIT_INTERVAL)
			Thread.sleep(waitTime);
		    else
			queueManager.moveToEndOfList(workingListName, errorListName, jobString, job.toString());
		} catch (InterruptedException e) {
		    log.error("Interrupted exception occured at RetryCreateBitcodinJobTask");
		}
		...
		if (job.getDrmType() != null) {
			...
			log.error("An error occured while fetching DRM configuration for retry count "
				+ job.getRetryCount(), e);
