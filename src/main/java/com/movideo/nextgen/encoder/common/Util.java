...
public class Util {
	...
	Gson gson = new Gson();
	/*
	 * EncodingJob job = new EncodingJob(); if(json.has("encodingJobId")){
	 ...
	 * job.setVariant(json.getString("variant")); if(json.has("errorType")){
	 * job.setErrorType(json.getString("errorType")); }
	 * if(json.has("drmType")){ job.setDrmType(json.getString("drmType")); }
	 * 
	 * 
	 * ArrayList<String> manifestTypes = new ArrayList<String>();
	 * 
	 * JSONArray manifestArray = json.getJSONArray("manifestTypes"); int len
	 * = manifestArray.length(); String[] result = new String[len];
	 * 
